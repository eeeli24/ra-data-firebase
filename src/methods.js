...
const delMany = async (ids, resourceName) => {
    await ids.map(id =>
        firebase
            .database()
            .ref(`${resourceName}/${id}`)
...
const getList = (params, resourceName, resourceData) => {
    if (params.pagination) {
        ...
        if (params.filter) {
            values = filterResource(values, params.filter);
        }
        if (params.sort) {
...
const getMany = (params, resourceName, resourceData) => {
    ...
    if (params.filter) {
        data = filterResource(data, params.filter);
    }
    return {data, ids: params.ids};
};

const filterResource = (resourceDataValues, filter) => {
    let items = resourceDataValues;
    ...
    if (filter) {
        ...
        items = items.filter(item => {
            ...
            filterKeys.forEach(filterKey => {
                const filterValue =
                    typeof filter[filterKey] === 'string' ? filter[filterKey].toLocaleLowerCase() : filter[filterKey];
                const itemFieldValue = item[filterKey];
                if (typeof itemFieldValue === 'string' && filterValue && valid) {
                    valid = itemFieldValue.indexOf(filterValue) >= 0;
                }

                if (typeof itemFieldValue === 'number' && !isNaN(filterValue) && valid) {
                    valid = itemFieldValue === parseFloat(filterValue);
                }
                if (filterKey === 'q' && filterValue && valid) {
                    valid =
                        JSON.stringify(Object.values(item).filter(i => typeof i === 'string'))
                            .toLocaleLowerCase()
